import { Sandbox } from "@typescript/sandbox/src"

export const createTwoslashInlayProvider = (sandbox: Sandbox) => {
  const provider: import("monaco-editor/esm/vs/editor/editor.api").languages.InlayHintsProvider = {
    provideInlayHints: async (model, _, cancel) => {
      const text = model.getValue()
      const queryRegex = /^\s*\/\/\s*\^\?$/gm
      let match
      const results: import("monaco-editor/esm/vs/editor/editor.api").languages.InlayHint[] = []
      ...
      while ((match = queryRegex.exec(text)) !== null) {
        ...
        const inlay: import("monaco-editor/esm/vs/editor/editor.api").languages.InlayHint = {
          // @ts-ignore
          kind: 0,
