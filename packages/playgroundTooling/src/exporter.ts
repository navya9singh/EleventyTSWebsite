...
type CompilerOptions = import("monaco-editor/esm/vs/editor/editor.api").languages.typescript.CompilerOptions

export const createExporter = (sandbox: Sandbox, monaco: typeof import("monaco-editor/esm/vs/editor/editor.api"), ui: UI) => {
...
**Playground Link:** [Provided](${fullURL})
  ...
  async function copyAsMarkdownIssue(e: JSX.TargetedMouseEvent<HTMLElement>) {
    //e.persist()
  ...
  function copyForChat(e: JSX.TargetedMouseEvent<HTMLElement>) {
    const query = sandbox.createURLQueryWithCompilerOptions(sandbox)
    const fullURL = `${document.location.protocol}//${document.location.host}${document.location.pathname}${query}`
  ...
  function copyForChatWithPreview(e: JSX.TargetedMouseEvent<HTMLElement>) {
    //e.persist()
