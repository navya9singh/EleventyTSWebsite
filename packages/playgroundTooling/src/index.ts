type Sandbox = import("@typescript/sandbox/src").Sandbox
...
export interface PlaygroundPlugin {
  ...
  modelChanged?: (sandbox: Sandbox, model: import("monaco-editor/esm/vs/editor/editor.api").editor.ITextModel, container: HTMLDivElement) => void
...
export const setupPlayground = (
  ...
  sandbox.setDidUpdateCompilerSettings(async () => {
    ...
    if (isESMviaModule || isESMviaModuleRes) {
      ...
      const nextRes = (moduleNumber === 199 || moduleNumber === 100 ? 99 : 2) as import("monaco-editor/esm/vs/editor/editor.api").languages.typescript.ModuleResolutionKind
  ...
  if (document.getElementById("config-container")) {
    //createConfigDropdown(sandbox, monaco) //tbd
    //updateConfigDropdownForCompilerOptions(sandbox, monaco) //tbd
  }
  ...
  // Set the errors number in the sidebar tabs //tbd
