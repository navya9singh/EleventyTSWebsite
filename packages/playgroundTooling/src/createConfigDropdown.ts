...
export const createConfigDropdown = (sandbox: Sandbox, monaco: Monaco) => {
  ...
  boolOptions.forEach(opt => {
    let summary;
    for (let i = 0; i < optionsSummary.length; i++) {
      if (opt.name === optionsSummary[i].id) {
        summary = optionsSummary[i];
        break;
      }
    }
  ...
  let target = optionsSummary[0];
  for (let i = 0; i < optionsSummary.length; i++) {
    if (optionsSummary[i].id === "target") {
      target = optionsSummary[i];
      break;
    }
  }
  const targetSwitch = createSelect(target.display, "target", target.oneliner, sandbox, sandbox.ts.ScriptTarget)
  dropdownContainer.appendChild(targetSwitch) //tbd
