...
import * as sandbox from "@typescript/sandbox/src"
import * as tsWorker from "@typescript/sandbox/src/tsWorker"
...
import * as playgroundPackage from "@typescript/playground-tooling/src"
import { setupPlayground} from "@typescript/playground-tooling/src"
import * as playgroundWorker from "@typescript/playground-worker"
import * as main from "monaco-editor/esm/vs/editor/editor.api"
...
//const Play: preact.FunctionalComponent<Props> = (props) => {
  ...
  useEffect(() => {
    ...
    getLoaderScript.src = `https://www.typescriptlang.org/js/vs.loader.js`;  //tbd
    ...
    getLoaderScript.onload = async () => {
      ...
      console.log("urlForMonaco", urlForMonaco)
      ...
      //var main: typeof import("monaco-editor")
      (async () => {
        //const ts = ts
        const playground = typeof playgroundPackage
        ...
        const sandboxEnv = await sandbox.createTypeScriptSandbox({
          text: localStorage.getItem('sandbox-history') || (playCopy.play_default_code_sample),
          compilerOptions: {},
        ...
        setupPlayground(sandboxEnv, main, playgroundConfig, CSSTranslate as any, Preact) //tbd
    ...
    <div id="parent-container">
      ...
      <nav className="navbar-sub">
        <ul className="nav">
          ...
          <li className="dropdown">
           <a id="compiler-options-button" href="#" className="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="menu" aria-expanded="false" aria-controls="compiler-options-dropdown" style={{ overflow: "hidden", whiteSpace:"nowrap" }}>{playCopy.play_subnav_config} <span className="caret"></span></a>
            <div id="compiler-options-dropdown" className="dropdown-dialog" aria-labelledby="compiler-options-button">
              <h3>{playCopy.play_subnav_config}</h3>
              <div className="info" id="config-container">
                <button className="examples-close">{playCopy.play_subnav_examples_close}</button>
                ...
                <div id="compiler-dropdowns">
                  <label className="select">
                    ...
                    <span className="compiler-flag-blurb">{playCopy.play_config_language_blurb}</span>
                  </label>
                </div>
          ...
          <li className="dropdown">
            <a href="#" id="examples-button" className="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="menu" aria-expanded="false" aria-controls="examples">{playCopy.play_subnav_examples} <span className="caret"></span></a>
            <div className="dropdown-dialog" id="examples" aria-labelledby="examples-button">
              <button className="examples-close" aria-label="Close dropdown" role="button">{playCopy.play_subnav_examples_close}</button>
          ...
          <li className="dropdown">
            <a href="#" id="handbook-button" className="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="menu" aria-expanded="false" aria-controls="examples">{playCopy.play_subnav_handbook} <span className="caret"></span></a>
          </li>
        </ul>
      ...
      <div className="raised" style={{ paddingTop: "0", marginTop: "0", marginBottom: "3rem", paddingBottom: "1.5rem" }}>
        <div id="loader">
          ...
          <p id="loading-message" role="status">{playCopy.play_downloading_typescript}</p>
        </div>
            ...
            <div id="editor-toolbar" className="navbar-sub" >
              ...
              <ul>
                <li id="versions" className="dropdown" >
                  <a href="#" data-toggle="dropdown" role="button" aria-haspopup="menu" aria-expanded="false" aria-controls="versions-dropdown" id='versions-button' style={{ overflow: "hidden", whiteSpace:"nowrap" }}>{playCopy.play_downloading_version}... <span className="caret" /></a>
                  <ul className="dropdown-menu versions" id="versions-dropdown" aria-labelledby="versions-button"></ul>
                </li>
                <li><a id="run-button" href="#" role="button">{playCopy.play_toolbar_run}</a></li>

                <li className="dropdown">
                  <a href="#" id="exports-dropdown" className="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" aria-controls="export-dropdown-menu" style={{ overflow: "hidden", whiteSpace:"nowrap" }}>{playCopy.play_toolbar_export} <span className="caret"></span></a>
                  <ul className="dropdown-menu" id='export-dropdown-menu' aria-labelledby="whatisnew-button">
                    <li><a href="#" onClick={() => playground.exporter.exportAsTweet()} aria-label={playCopy.play_export_tweet_md} >{playCopy.play_export_tweet_md}</a></li>
                    <li role="separator" className="divider"></li>
                    <li><a href="#" onClick={(e: any) => playground.exporter.copyAsMarkdownIssue(e)} aria-label={playCopy.play_export_copy_md} >{playCopy.play_export_copy_md}</a></li>
                    <li><a href="#" onClick={(e: any) => playground.exporter.copyForChat(e)} aria-label={playCopy.play_export_copy_link}  >{playCopy.play_export_copy_link}</a></li>
                    < li > <a href="#" onClick={(e: any) => playground.exporter.copyForChatWithPreview(e)} aria-label={playCopy.play_export_copy_link_preview}  >{playCopy.play_export_copy_link_preview}</a></li>
                    < li role="separator" className="divider" ></li>
                    <li><a href="#" onClick={() => playground.exporter.openInTSAST()} aria-label={playCopy.play_export_tsast}>{playCopy.play_export_tsast}</a></li>
                    <li><a href="#" onClick={() => playground.exporter.openInBugWorkbench()} aria-label={playCopy.play_export_bugworkbench}>{playCopy.play_export_bugworkbench}</a></li>
                    <li><a href="#" onClick={() => playground.exporter.openInVSCodeDev()} aria-label={playCopy.play_export_vscode_dev_play}>{playCopy.play_export_vscode_dev_play}</a></li>
                    <li role="separator" className="divider"></li>
                    <li><a href="#" onClick={() => playground.exporter.openProjectInCodeSandbox()} aria-label={playCopy.play_export_sandbox} >{playCopy.play_export_sandbox}</a></li>
                    <li><a href="#" onClick={() => playground.exporter.openProjectInStackBlitz()} aria-label={playCopy.play_export_stackblitz} >{playCopy.play_export_stackblitz}</a></li>
                  </ul>
                </li>
                <li><a id="share-button" href="#" role="button">{playCopy.play_toolbar_share}</a></li>
              </ul>
